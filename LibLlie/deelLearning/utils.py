...
from LibLlie.deelLearning.config import *
...
def save_img(tensor, output_dir, name, save_format='png'):
...
def scriptDL(
    ...
    if os.path.isdir(input):
        ...
        for imTensor, name in tqdm(testImages, desc=f'using {os.path.basename(model_path)} to infer', unit='image'):
            ...
            save_img(output, output_dir, name=name[0], save_format=save_format)
        ...
        model.eval()
        imTensor = imTensor.to(device)
        result, _ = model(imTensor)

        if save_image == True:
            save_img(result, output_dir, name=name, save_format=save_format)
        if show_image == True:
            show_img(result)
